...
<div id="introduction">
	You've likely encountered one of those cards or widgets displaying the most
	recent or current song you're listening to. In this guide, I'll take you
	through the process step by step on how to build it. We'll start by creating
	the app on Spotify, generating the necessary tokens, and then proceed to
	implement the server-side API calls on SvelteKit. Additionally, we'll cover
	the creation of the related frontend component that will utilize these API
	calls.
</div>
