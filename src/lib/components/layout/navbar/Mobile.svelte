...
<div
	class="fixed top-0 left-0 z-10 h-full w-full transform bg-background duration-500 ease-[cubic-bezier(0.7,0,0,1)] md:hidden"
...
>
	<div class="mt-6 flex items-center justify-between px-8">
		<a href={resolve('/')} aria-label="Home page logo" class="fill-foreground" onclick={toggleNav}>
			<Logo />
		</a>
		<button
			aria-label="Close navigation menu"
			onclick={toggleNav}
			class="rounded-full border border-elevation-one p-2 duration-500 md:hidden"
